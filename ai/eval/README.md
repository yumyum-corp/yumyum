...
`샌드위치_닭가슴살`(240 kcal/100g)이다. 그래서 `mfds_lookup`은 **이름이 정확히
일치하는** 레코드만 모으고, 없으면 조회 실패로 센다. 틀린 음식의 계수를
쓰느니 실패가 정직하다.

**스캔 범위가 결론을 바꾼다.** 원물 단품이 검색 결과의 뒤쪽 블록에 몰려 있어서,
`닭가슴살`(총 3911건)·`우유`(2709건)·`아몬드`(2243건)의 정확일치가 모두
**17페이지에서 처음** 나왔다. 초기 구현은 5페이지(500건)까지만 훑어 이 셋을
"DB에 없음"으로 기록했고, 그 탓에 실패율이 35%로 부풀려졌다. 상한을 40페이지로
올리고, 일치를 찾은 뒤 5페이지 연속 새 게 없으면 멈추도록 했다(`_MFDS_QUIET_PAGES`).

정확 일치를 찾아도 한 건이 아니다. 평가셋의 정답명 20개로 실측(2026-09-12):

| 조회명 | 정확일치 | kcal/100g 범위 | 모호도 |
| --- | --- | --- | --- |
| 부대찌개 | 35건 | 12~228 | **19.0배** |
| 바나나 | 4건 | 82~454 | 5.5배 |
| 쫄면 | 23건 | 107~348 | 3.3배 |
| 비빔밥 | 10건 | 112~320 | 2.9배 |
| 규동 | 12건 | 136~270 | 2.0배 |
| 브로콜리 | 7건 | 20~31 | 1.6배 |
| 닭가슴살 | 2건 | 106~110 | 1.0배 |
| 방울토마토 · 삶은계란 · 삶은밤 · 해물야끼우동 | **0건** | 전량 스캔해도 부재 | — |

**정확일치 실패 20%(4/20), 성공해도 후보끼리 최대 19배 흩어진다.** 이 모호성
자체가 Option A의 실제 성능이므로 후보 수와 스프레드를 1급 지표로 기록한다.
...
**`prod` 프롬프트는 라우터에서 그대로 가져온다.** `app/routers/ai_meal.py`의
`PHOTO_PROMPT_TEMPLATE`을 import하므로 사본이 어긋날 일이 없다. 예전에는
하네스가 사본을 들고 있었고 "라우터를 바꾸면 이쪽도 같이 바꿔라"는 주석에
의존했는데, 그런 규약은 언젠가 깨지고 그러면 프로덕션이 아닌 것을 측정하게
된다. `--max-tokens` 기본값(800)도 라우터와 맞춰져 있다.

## 프롬프트 변주

| 이름 | 내용 |
| --- | --- |
| `prod` | 현행 프로덕션 프롬프트 (중복 금지 규칙 포함) |
| `prod_v1` | 중복 금지 채택 **이전** 프롬프트. 회귀 기준선 |
| `hinted` | `prod` + 그램 추정 근거(식기 표준 크기·1인분 표준량) 요구 |

`prod_v1` vs `prod` 비교가 중복 금지 규칙의 채택 근거다. 사진 31장 실측:
...
자세한 근거는 `docs/adr/2026-06-23-vision-ai-photo-meal.md`의 「결과 5·6」.
